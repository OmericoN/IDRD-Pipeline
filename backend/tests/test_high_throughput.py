...
class FakeRepo:
    ...
    def try_start_run_level_stage(self, pipeline_run_id, stage):
        assert pipeline_run_id == 9
        assert stage == PipelineStage.EXPORT_INSIGHTS
        return self.finalize_started
...
def test_dispatch_plan_finalizes_once_when_terminal(monkeypatch):
    ...
    FakeRepo.finalize_started = True
    monkeypatch.setattr(high_throughput, "PipelineRepository", FakeRepo)
...
def test_finalize_uses_completed_with_errors_outcome(monkeypatch):
    FakeRepo.outcome = "completed_with_errors"
    FakeRepo.finished = []
    monkeypatch.setattr(high_throughput, "PipelineRepository", FakeRepo)
    monkeypatch.setattr(
        high_throughput.services,
        "export_insights_csv",
        lambda output_path, pipeline_run_id=None: {"status": "successful", "output_path": output_path},
    )

    result = high_throughput.finalize_high_throughput_run("storage/exports/insights.csv", 9)

    assert result["status"] == "completed_with_errors"
    assert FakeRepo.finished[-1] == {"pipeline_run_id": 9, "status": "completed_with_errors"}
