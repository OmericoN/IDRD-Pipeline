...
from idrd.domain.results import ConversionResult, DownloadResult
...
def main() -> None:
    ...
    if args.command == "worker-command":
        print("uv run celery -A idrd.infrastructure.worker.celery_app:celery_app worker --loglevel=INFO --pool=solo")
        return
    ...
    if args.command == "import-um-datasets":
        from idrd.application import pipeline_services as services
...
def _doctor() -> dict[str, Any]:
    from idrd.application.orchestrator import celery_worker_available
